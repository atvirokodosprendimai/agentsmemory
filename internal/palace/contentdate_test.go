...
func TestExtractContentDate(t *testing.T) {
	cases := []struct {
		...
		{"filename iso wins", "2024-11-08-notes.md", "no date in body", "2024-11-08"},
		{"filename natural language", "April-6th-2011-notes.md", "body", "2011-04-06"},
		...
		{"no date", "plain.txt", "just some prose with no dates", ""},
		{"only beyond first lines ignored", "", "l1\nl2\nl3\nl4\nl5\nl6\nl7\nl8\nl9\nl10\nl11 2024-11-08", ""},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := extractContentDate(tc.source, tc.in); got != tc.want {
				t.Fatalf("extractContentDate(%q, %q) = %q, want %q", tc.source, tc.in, got, tc.want)
			}
		})
	}
}
