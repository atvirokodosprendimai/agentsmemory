...
*"am I asked anywhere to stop, ever?"* ⚠ "Never stop" is bounded by the `deny`
list in `.claude/settings.json`, not by judgement — and on 2026-09-04, at the
owner's instruction, that list SHRANK to three entries: `git push --force:*`,
`git push -f:*` and `rm -rf /*`. Nine commands moved to `allow` the same day:
`gh pr merge`, `gh release create`, `gh release delete`,
...
Nothing equivalent gates a release or a destructive local command; there the
session's own judgement is the last thing standing.
...
**AND A KNOB READ IN ONE ENTRY POINT IS INERT IN EVERY OTHER ONE THAT OFFERS IT.**
`--otel-endpoint` was declared in `dataFlags`, so it appeared in the help of every
command carrying those flags, while `telemetry.Setup` ran in `run()` and `runEval()`
only: everywhere else the flag parsed, reached `cfg.OTELEndpoint`, and installed no
provider, so every span was a `nonRecordingSpan` and nothing was exported —
silently, and with `README.md` and `.env.example` both promising otherwise. Every
existing gate passed, because each asks whether the field is assigned and read
SOMEWHERE, which is the weaker question ADR-006 already rejected; measured, the
whole feature could be cut out of the SERVING path with a green suite. The remedy
is a seam rather than a detector: `withTelemetry` is the one call site,
