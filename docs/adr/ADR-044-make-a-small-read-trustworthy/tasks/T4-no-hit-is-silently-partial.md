...
⚠ **A CORRECTION WAS FILED HERE AND THEN WITHDRAWN. Both are left visible on purpose.** Later the
same day a probe appeared to show the search path serving one chunk of fourteen at
`content_coverage: 1.000`, and this table's conclusion was marked FALSE on that evidence. It was the
FIXTURE: 25 memories built from `strings.Repeat` of one sentence have byte-identical chunks, which
collapse under content-addressed dedupe, so each memory really did hold one chunk by the time it was
read. Re-run with per-memory filler, every memory reassembles and coverage is correct. **The sentence
below stands.** The residue is filed in `BACKLOG.md` as a narrower and genuine finding about
cross-memory chunk dedupe, and the retraction is kept there rather than deleted.
