...
**Cross-references:** ADR-022 (**partial dependency, narrowed** — its subject addressing is *derived*, so every drawer already has an address and the naming half costs this ADR nothing; what remains a hard prerequisite is its adjacency fix, without which referrals carry no information), ADR-001 (recall answers or abstains — the abstain decision this ADR needs and does not duplicate; note its T3 block, addressed below), ADR-019 (the agent sees a quarter of the memory — the payload budget this ADR spends differently), ADR-013 (a page of memories, not chunks)
...
**Compressing the gloss in the AAAK dialect.** Open, deliberately, and worth stating because ADR-022 rejects AAAK for the *subject* and the two questions have different answers. A subject is an identifier and must resolve globally, so AAAK's context-resolved three-letter codes alias (`ARC` — architecture or archive?); the gloss is prose, which is exactly what AAAK compresses, and it sits on the token-critical path where three referrals each carry a description. So terseness pays here and does not pay there. Not decided now because the gloss has no measured cost yet: it is bounded by the registry's one line per token, and shrinking a bounded cost before measuring it is the optimisation this ADR's own falsification is designed to catch.
