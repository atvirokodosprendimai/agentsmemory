...
| T4 | Perform the recall at compaction and inject the result | pending | F-6 | `apk add --no-cache bash git >/dev/null && go test ./clients/claude-code/ -run "^(TestF6AHookIsSilentInTheCommonCase\|TestPreCompactHookIsRegistered)$" -count=1 -v` |
