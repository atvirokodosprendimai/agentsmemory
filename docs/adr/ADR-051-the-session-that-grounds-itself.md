...
**2. A structured hook reaches the model on events whose stdout is discarded.**
The injecting/discarded split governs *plain stdout only*. The reference states `additionalContext` support for `PreToolUse` ("`permissionDecision`, `updatedInput` and `additionalContext` — add context Claude sees"), `PostToolUse` (`updatedResult` or `additionalContext`) and `PostToolUseFailure`. So the twenty-seven unused events are mostly **not closed doors**, and the mechanism is one this project already ships blind: `agentsmemory-subagent-start-hook.sh` declares `hook-output: structured` and injects on `SubagentStart`, an event whose plain stdout goes nowhere.
...
- Restarting ADR-041's T3, T4 or T5. (permanent: fact: re-measured 2026-09-03 over the MCP tool against the container built from main 799769d, a bare identifier tops out in a narrative or scratch room 56% of the time against 0% for a real question, so it remains a disqualifying query for a semantic cue; citation: file `docs/adr/ADR-051-the-session-that-grounds-itself/tasks/T2-anchor-cue-at-pretooluse.md:11`)
...
- [ ] Re-measure `SubagentStart` `additionalContext` delivery with a discriminator only the hook text can satisfy. The 2026-09-03 measurement is a subagent's *report* of receipt, not a verbatim confirmation, and a pointed question invites an agreeable answer. (deferred: T3's Verification Log)
