...
- [x] Correct `clients/claude-code/bootstrap.md`, which SHIPS the sentence T4 just made false (done 2026-08-29 — and the Follow-up named ONE copy of at least FOUR, which review caught. Fixed: `clients/claude-code/bootstrap.md`, gated by `TestTheShippedProtocolDoesNotSayAFragmentIsUnmarked`; and `internal/web/bootstrap-memory.md`, which said it TWICE and is `//go:embed`-ed and SERVED at `/bootstrap-memory` — the copy handed out by the very server whose behaviour changed — gated by `TestTheServedProtocolDoesNotSayAFragmentLooksComplete`. Both gates read the EMBEDDED asset, which is what ships. ⚠ STILL OPEN, AND DELIBERATELY NOT CORRECTED YET — the palace-side copies are live DATA and go false at DEPLOY, not at merge. **Measured 2026-08-29 against the running server:** `am_get_drawer` without `whole` returned chunk 0 of a 4-chunk memory with NO `content_truncated` and NO `content_length`, so T4 is not deployed and those sentences are still TRUE. Correcting them now would make them wrong in the other direction — telling every session to rely on a field the running server does not set. ⚠ **A SKILL CATALOGUE IS INSTANCE-SCOPED, and the copy to correct is a DIFFERENT skill in each palace** — established by two sessions disagreeing and both being right. This session is `mode: local`, workspace slug `local`; the reviewer's is `hosted`. Both catalogues hold 6 skills and their contents differ:

| Instance | The copy carrying the now-false claim | How to correct it |
|---|---|---|
| local (slug `local`) | `memory-orchestration` v4, Silent traps: *"Use `am_get_drawer(id, whole=true)`; otherwise one matching chunk looks complete."* | `am_update_skill`, licensed by that skill's own criterion ("a rule that became wrong") |
| hosted | `start-here` v13 (reported by the reviewer; `memory-orchestration` there is a retirement stub merged into it) | same, from a session on that instance |
| both | the `am_skillset` preamble | platform-owned; needs a superadmin edit |

An earlier draft of this line said `start-here` "does not exist in this workspace". The COUNT was right and the CONCLUSION was wrong: it does not exist in the LOCAL one. A claim that a skill is absent needs the instance named, the same way a defect needs the ref it is relative to — and the cost of the unqualified version is specific, since a session reading "no such skill" stops looking for the one document that explains how to enter a wing. Corrected on the reviewer's evidence): *"⚠ `am_get_drawer` RETURNS ONE CHUNK, AND IT LOOKS COMPLETE … Nothing marks the fragment as partial."* As of T4 something does — `content_truncated` with the memory's `content_length`. The installer writes this text into every agent's protocol, so a stale warning here teaches every new session to distrust a field that now works, and this record's own "documentation is load-bearing" principle applies to the half that ships. Not folded into T4: the task's Affected Files did not include it and quietly widening a task is how an acceptance fence stops describing what was done
