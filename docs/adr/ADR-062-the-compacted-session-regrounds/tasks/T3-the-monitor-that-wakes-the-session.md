...
| `clients/claude-code/hooks/agentsmemory-precompact-hook.sh` | drop a last user turn that names no work: one deny list of harness-generated openers — a bare `/` command, a wrapped `<command-…>` one, the `<local-command` family, a task notification, a system reminder, the post-compaction continuation preamble, a relayed peer message. Entries are matched by name, and each carries the transcript it was observed in |
...
8. [S8] Record one real compaction in which the monitor fires and the session re-grounds. [proof: human: no test can observe a notification arriving in a live session; only a transcript shows the wake changed what the session did next]
   ...
   **Observed 2026-09-05, session `a59e1cad`, monitor `bz4bkmjl3`, armed at `136004f` before the compaction.** The compaction replaced the context; the monitor's line arrived as a notification and the session took a turn on it. What it changed is the part no test could have shown: the re-ground was the FIRST action out of the summary, ahead of the two PR threads that were also waiting, and it was taken because the notification arrived — not because the summary asked for it. The printed `PAUSE` was in that same context and had been for every prior compaction; the wake is what got acted on.
   ...
   ⚠ **AND A SECOND CORPUS FOUND A FIFTH, WHICH THIS SESSION COULD NOT HAVE FOUND AT ALL.** Reported on PR #283: a reviewer ran the fixed extraction against a DIFFERENT session's real transcript and got `prompt=Another Claude session sent a message:…`, a peer session's message relayed as a plain `type=user` turn. Prose again, so no bracket rule reaches it, and on a session that talks to peers it can be the last plain turn for a long stretch. **It is not reproducible in this checkout** — this session has no peers, and the only occurrences of that string in its own transcript are the reviewer's report quoted back through a tool result. Accepted on their artifact rather than on a fixture, which is the point: three of the five forms came from running the code against a real transcript, and the fifth came from running it against somebody ELSE'S. The openers are now one deny list, each entry naming where it was seen, so the next reader can tell an observation from a guess.

   ⚠ **AND WRITING THAT LIST BROKE A GUARD THAT WORKED — CAUGHT ON THE NEXT REAL-TRANSCRIPT RUN, BY NOTHING ELSE.** Tidying `^<local-command` into `^<local-command-stdout` looked like naming the member precisely. The deployed hook's very next run against the live transcript returned `prompt=<local-command-caveat>Caveat: The messages below were generated…`, and the whole suite was green over it, because no fixture held that member either. The prefix covers a family whose members are not enumerable in advance, so it stays a prefix, and `local command caveat` is a subtest with its own mutant. The recurrence is worth more than the fix: the previous forms arrived by being MISSED, this one by being TIDIED, and only the same habit — run it against a real artifact, not the fixtures — found either.
...
- 2026-09-05 · 8fd462c* · mutant killed · exit 1 · `clients/claude-code/hooks/agentsmemory-precompact-hook.sh` · Drops the peer-message opener from the deny list. The peer_session_message subtest must go red naming the relayed message as the task in flight; every other case still passes, so the entry is bound to its own assertion rather than to the list as a whole. · acceptance-sha256:a7041b09bb5c6dda0c8df95d5a20437f8d9e710f3b533d108aeaf25d1d32395f · covers:harness chrome is not the task
- 2026-09-05 · 3191758* · mutant killed · exit 1 · `clients/claude-code/hooks/agentsmemory-precompact-hook.sh` · Re-narrows the bracketed-chrome prefix to the -stdout member, the exact regression this commit repairs. Only local_command_caveat goes red, so the family prefix is bound by its own assertion rather than by the sibling that shares its opening. · acceptance-sha256:a7041b09bb5c6dda0c8df95d5a20437f8d9e710f3b533d108aeaf25d1d32395f · covers:harness chrome is not the task
...
- 2026-09-05 · 8fd462c* · exit 0 · `gofmt -l clients/claude-code | grep -q . && exit 1 …` · acceptance-sha256:a7041b09bb5c6dda0c8df95d5a20437f8d9e710f3b533d108aeaf25d1d32395f · ms:7814
- 2026-09-05 · 8fd462c* · exit 0 · `gofmt -l clients/claude-code | grep -q . && exit 1 …` · acceptance-sha256:a7041b09bb5c6dda0c8df95d5a20437f8d9e710f3b533d108aeaf25d1d32395f · ms:7770
- 2026-09-05 · 3191758* · exit 0 · `gofmt -l clients/claude-code | grep -q . && exit 1 …` · acceptance-sha256:a7041b09bb5c6dda0c8df95d5a20437f8d9e710f3b533d108aeaf25d1d32395f · ms:8012
