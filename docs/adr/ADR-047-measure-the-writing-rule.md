...
Five properties make it an instrument rather than a way to confirm what the skills already say.
...
4. **The judge is blind to the policy.** It receives the question, the gold answer and the candidate answer, and nothing that names which cell produced it. Stated because the judge prompt is the one place a preferred outcome could be smuggled in for free.
...
**What would make that rule fail, and whether such data exists.** It fails whenever a policy's interval spans zero, which is the outcome for any policy that does nothing — and LongMemEval-S at n=500 across six question types, split in half, is small enough that a modest effect will not clear it. That is deliberate: the rule is falsifiable on the data we have, and the most likely single outcome is that some rules `start-here` states with confidence turn out to be neutral. The rule is valid **for** this corpus, this reader/judge model, this context budget and this ranking profile; a threshold is always valid for a configuration, and the run writes all four into its results file.
...
| `--data`, `--wing`, `--write`, `--query`, `--n`, `--context-tokens`, `--out` flags | add | `cmd/server/longmemeval.go` | the run; `TestEveryFlagIsRead` |
...
| The subset is small enough that everything is neutral | High | Med | This is a legitimate result and is reported as one; `--n` and the subset are in the results file so a later, larger run is comparable |
