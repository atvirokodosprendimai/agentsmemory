...
|---|---|---|
| `questionGen` — Ollama `POST /api/generate` (`{model, prompt, stream, options}` in, `{response}` out) | `cmd/server/eval.go:810-1057` | **Yes, and extracted.** It is the reader and the judge. `cmd/server/kgextract.go:214-290` holds a second copy of the same TRANSPORT; this ADR moves one implementation into `internal/gen` rather than writing a third. ⚠**Corrected 2026-09-01 while executing T3.** This row previously read "with an OpenAI-compatible `/v1` branch (`openAIShaped()`)", which is false and was written from the method's NAME rather than its call sites: `openAIShaped()` has exactly one caller, `hint()`, where it selects the wording of a failure message. `ask()` never branches on it and always posts `/api/generate` — as `questionGen`'s own doc comment says out loud, *"It is NOT OpenAI-shaped … deliberately out of scope here"*. Left standing, this row would have made T3 add an OpenAI request path under the description of a behaviour-preserving move. |
