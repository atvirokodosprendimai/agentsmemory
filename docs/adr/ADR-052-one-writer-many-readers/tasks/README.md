...
|----|-------|--------|--------|------------|
| T1 | Reproduce the lock upgrade failure as a red test | pending | — | `go test ./cmd/server/ -run 'TestAReadThenWriteTransactionSurvivesConcurrentWriters$'` |
| T2 | One DSN per role, and a writer that takes its lock at BEGIN | pending | — | `go test ./cmd/server/... -count=1` |
