...
**Proof map:** v1
**Rests-on:** `the thin view carries no content`, `the argument is what selects the thin view`, `the thin hit says it is partial`
...
```bash
set -o pipefail
go test ./internal/mcptest/ -run 'TestAnIdsOnlyPageCarriesNoContentAndSaysSo$' -count=1 2>&1 | tee /tmp/acc.out && ! grep -qE "no tests to run|^FAIL|^--- FAIL" /tmp/acc.out && \
