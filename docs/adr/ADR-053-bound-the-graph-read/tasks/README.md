...
| T2 | Containment edges are a listing, not a fact | done | — | `go test ./... -count=1` |
| T3 | Tell the writer when a node has outgrown its tier | done | — | `go test ./internal/palace/... ./internal/mcpserver/... -count=1` |
| T4 | A fetch carries the facts about what it returns | done | — | `go test ./internal/mcpserver/... -count=1` |
