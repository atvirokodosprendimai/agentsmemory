...
**Where this project stands:** at parity on the retrieval stack (hybrid + rerank is now table stakes; Zep's "no LLM reranking" is a choice this project also made — a cross-encoder, not a generator). Ahead on scoping discipline: the wing/craft split with a measured reason (a bigger corpus adds competitors, ADR-058 §Context) is the same conclusion Zep reached by refusing to merge group and user graphs.
