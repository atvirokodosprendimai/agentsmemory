...
| F-7 | `serverInstructions` stays within its measured ceiling; any cue replaces text rather than adding to it. | `internal/mcpserver/instructions_test.go::TestInstructionsStayShort` | @implemented | |
...
| F-11 | The MCP instructions name the CLASS OF CLAIM that requires a recall, and contain no bare instruction to recall before acting. | `internal/mcpserver/recallcue_spec_test.go::TestF11InstructionsNameTheClassOfClaimNotTheDuty` | @implemented | Shipped by T6 2026-08-28. `serverInstructions` names the class of claim — the three shapes F-2 defines the unit by, and the reason source cannot settle it — and the bare imperative is REPLACED rather than joined, which is what F-8 requires of a mechanism. The binding was a self-retiring skip while T6 was blocked; the skip is gone and the test now asserts the imperative is ABSENT. |
...
**No-change assertion** — a claim that the current state equals the past state, about a subject that resolves in this repository. **Resolves** — the named symbol, path or record exists in the working tree, so the palace could hold a decision about it. **Recall** — an `am_search` or `am_get_drawer` call. **Preceded** — a recall occurring after the last USER TURN before the assertion; a tool result is not a user turn. *(Amended 2026-08-28 by the owner. The term was left undefined and the implementation latched at the first recall of a session and never reset, so it answered "did this session ever touch the palace" — 52.8% where this reading gives 7.6% over one corpus. All three candidate readings were measured before choosing; see BACKLOG. Per F-16 the baseline was re-taken and the version stamp bumped to v3, which now covers the definition of preceded as well as the sentence classifier.)* **Pushed content** — palace content that enters context without the agent asking. **Pulled content** — content the agent must call for. **Mechanism** — a change intended to raise the rate, excluding additional protocol prose (F-8). **Observation** — one session's counts: assertions seen, assertions preceded by a recall, classifier version. **Compliance-dependence** — how much a mechanism relies on the agent choosing to act; the ordering axis for shipping.
