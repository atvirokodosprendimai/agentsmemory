...
| `/am`, `/load-skill` + Claude's ten hooks | Task-scoped grounding, the end-of-turn checkpoint that stops memory being lost, a `UserPromptSubmit` recall that asks the palace about each task in the user's own words, and the two that make a Claude SUBAGENT a session: `SubagentStart` puts the recall instruction next to its task, `SubagentStop` asks it for what it found. A `UserPromptExpansion` recall covers the turns the `UserPromptSubmit` one refuses by design — a slash command's literal text is a command name, so the recall runs against what it expands into. A `PostToolUse` recorder lets the end-of-turn nudge name the files the session actually changed. A `PreToolUse` anchor cue surfaces the memories pinned to a file as it is opened — no query, an exact join on the path, and silent when nothing pins it. A `PreCompact` note-writer records branch, HEAD, uncommitted count and the touched files in the last moment before a compaction, and the `SessionStart` recall hands it back on the other side. Codex currently installs only the proven `Stop` hook. | Same installer |
...
| `PreToolUse` / `PostToolUse` / `PreCompact` / `UserPromptExpansion` | ✅ | ❌ | ❌ | ❌ | ❌ |
